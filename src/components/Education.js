...
function Education() {
  ...
  return (
    <div className="md:my-8 my-5">
      <h1 className="text-xl md:text-3xl text-center text-yellow-400 font-bold mb-4">
        Education
      </h1>
      <div className="flex flex-col md:flex-row items-center justify-center mx-4 gap-6">
        <div className="relative flex flex-col gap-6 p-8 bg-gray-800 rounded-xl shadow-lg w-full max-w-4xl">
          ...
          {education.map((edu, index) => (
            <div key={index} className="flex gap-6 items-start relative">
              <div className="relative flex flex-col items-center">
                <div className="bg-yellow-500 text-gray-900 rounded-full flex items-center justify-center p-3 transition duration-300">
                  <MdOutlineAccessTime className="h-6 w-6" />
                </div>
                {index < education.length - 1 && (
                  <div className="absolute top-12 w-0.5 bg-yellow-500 h-32 md:h-16"></div>
                )}
              </div>
              <div className="flex flex-col w-full">
                <p className="text-lg font-semibold text-yellow-400">{edu.institution}</p>
